...
import type { AssignEmployeesRequest } from "..";
import { ConstructionSiteApi } from "../api/construction-site.api";
import { constructionSitesKeys } from "../api/construction-site.keys";

export function useAssignEmployeesToConstructionSite() {
  const navigate = useNavigate();
  const qc = useQueryClient();
  const { enqueueSnackbar } = useSnackbar();

  return useMutation({
    mutationFn: (payload: AssignEmployeesRequest) =>
      ConstructionSiteApi.assignEmployees(payload),
